...
// acme is the org every record in these tests belongs to.
const acme = "acme"
...
func TestAlertStoreEviction(t *testing.T) {
	...
	for i := 0; i < 200; i++ {
		...
		store.Add(txID, []types.Alert{{ID: fmt.Sprintf("alert-%d", i), OrgID: acme}})
	}
	...
	if alerts := store.ByTx(acme, "tx-0"); len(alerts) > 0 {
		t.Error("tx-0 should have been evicted")
	}

	// Most recent entries should still exist.
	if alerts := store.ByTx(acme, "tx-199"); len(alerts) == 0 {
...
func TestAlertStoreNoEvictionUnderCapacity(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		store.Add(fmt.Sprintf("tx-%d", i), []types.Alert{{ID: fmt.Sprintf("a-%d", i), OrgID: acme}})
	}
	...
	for i := 0; i < 50; i++ {
		if alerts := store.ByTx(acme, fmt.Sprintf("tx-%d", i)); len(alerts) == 0 {
...
func TestAlertStoreDuplicateTxID(t *testing.T) {
	...
	store.Add("tx-1", []types.Alert{{ID: "a1", OrgID: acme}})
	store.Add("tx-1", []types.Alert{{ID: "a2", OrgID: acme}})

	if store.Len() != 1 {
		t.Errorf("store size = %d, want 1 (same tx_id)", store.Len())
	}

	alerts := store.ByTx(acme, "tx-1")
	if len(alerts) != 2 {
